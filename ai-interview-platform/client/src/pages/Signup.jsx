...
import { User, Mail, Lock, ShieldAlert, ArrowRight, Eye, EyeOff, Loader2 } from 'lucide-react';
...
export default function Signup({ setToken, setUser, setCurrentTab }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        triggerToast('Registration complete! Generating token keys...', 'success');
        ...
        triggerToast(data.message || 'Registration request could not be processed', 'error');
      }
    } catch (err) {
  ...
  return (
    <div className="max-w-md mx-auto my-12 relative">
      ...
      {/* Glow Effects */}
      <div className="absolute -inset-1 bg-gradient-to-r from-purple-500 via-indigo-600 to-cyan-500 rounded-3xl blur opacity-25 group-hover:opacity-100 transition duration-1000 group-hover:duration-200"></div>
      ...
      <div className="relative glass-panel p-8 rounded-3xl border-indigo-950/40 space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <h2 className="text-2xl font-extrabold font-outfit text-white tracking-tight">
            Register Candidate Key
          </h2>
          <p className="text-xs text-slate-400">
            Create an verified endpoint profile to secure assessment archives.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1">
            <label className="text-[11px] font-bold uppercase tracking-wider text-slate-400 font-mono">Full Name</label>
            <div className="relative">
              <User className="absolute left-3.5 top-3.5 w-4 h-4 text-slate-500" />
              <input
                type="text"
                placeholder="Soumadeep Dev"
                value={name}
                onChange={(e) => {
            ...
            {errors.name && <span className="text-[10px] text-rose-400 font-mono font-semibold">{errors.name}</span>}
          </div>
          ...
          {/* Password field */}
          <div className="space-y-1">
            <label className="text-[11px] font-bold uppercase tracking-wider text-slate-400 font-mono">Secure Access Key</label>
            <div className="relative">
          ...
          >
            {loading ? (
              <>
                ...
                <span>Generating Hashed Salt Keys...</span>
              </>
            ) : (
              <>
                <span>Secure Endpoint Keys</span>
