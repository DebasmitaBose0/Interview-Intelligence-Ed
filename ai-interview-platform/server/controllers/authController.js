  ...
  try {
    ...
    try {
      ...
      if (!result || !result.success) {
        await OTP.deleteMany({ email });
        logger.error('Failed to dispatch password reset OTP email', { email, error: 'Notification service returned failure' });
  ...
  try {
    ...
    try {
      ...
      if (fbUser) {
        ...
        logger.info(`[Firebase Auth] Password updated for: ${email}`);
      }
    } catch (fbErr) {
      logger.warn(`[Firebase Auth] Could not sync password: ${fbErr.message}`);
    }
    ...
    logger.warn('Failed OTP verification attempt', { email: req?.body?.email, error: error.message });
